...
local ELEMENT_TYPE_GENERICINST = 0x15
...
function getDotNetInfoList()
  return frmDotNetInfos  
end
...
settings.StartParsingOnFirstShow=settings.s['ParseOnShow']=='1'
...
local function SpawnConfigDialog()
  if frmDotNetInfoConfig==nil then
    ...
    frmDotNetInfoConfig.registerFirstShowCallback(function(f)
      ...
      if monopipe==nil then
        frmDotNetInfoConfig.cbSkipSafetyCheck.Visible=false
      end
      f.endUpdateBounds()
    end)
  end
  ...
  frmDotNetInfoConfig.cbParseInBackground.checked=settings.StartParsingOnFirstShow
  ...
  if frmDotNetInfoConfig.showModal()==mrOK then
    ...
    settings.StartParsingOnFirstShow=frmDotNetInfoConfig.cbParseInBackground.checked
    ...
    settings.s['StartParsingOnFirstShow']=settings.StartParsingOnFirstShow and '1' or '0'
  end
end
...
local function getClassParent(Class)
  if Class.Image==nil then
    OutputDebugString('Generated class')
    return nil
  end
  ...
  if Class.Parent==nil then
    ...
    if Class.ParentHandle and (Class.ParentHandle~=0) then      
      if Class.Image.Domain.Control==CONTROL_MONO then
        ...
        if Class.Parent.Name==nil or ImageHandle==nil then
          Class.Parent=nil 
          return nil
        end
  ...
  if Class.Image.Domain.Control==CONTROL_MONO then  
    ...
    if classdata and classdata.Fields then
      for i=1,#classdata.Fields do
        ...
        e.Class=Class  
        e.Address=classdata.Fields[i].Address --ce 7.7+
...
local function StartClassFetch(frmDotNetInfo, Image, Filter, FetchDoneFunction)
  ...
  if tonumber(frmDotNetInfo.ProcessID)~=getOpenedProcessID() then 
  ...
  if frmDotNetInfo.ClassFetchThread then
    ...
    if frmDotNetInfo.ClassFetchThread.waitfor(4000) then    
      frmDotNetInfo.ClassFetchThread.destroy()
      frmDotNetInfo.ClassFetchThread=nil
    else
      frmDotNetInfo.ClassFetchThread.FreeOnTerminate(true)
      frmDotNetInfo.ClassFetchThread=nil
    end
  ...
  if Image.Classes==nil then
    if (Image.Domain.Control==CONTROL_MONO) and mono_skipsafetycheck==false and isPaused() then  
      FetchDoneFunction()
      return
    end
...
local function FillClassInfoFields(frmDotNetInfo, Class)
  --if tonumber(frmDotNetInfos[frmDotNetInfo.Tag].ProcessID)~=getOpenedProcessID() then 
  if tonumber(frmDotNetInfo.ProcessID)~=getOpenedProcessID() then 
    return 
  end
      ...
      if Class.Fields[i].Static then
        ...
        if frmDotNetInfo.lvFields2 then
         ...
         -- printf("Class.Fields[%d].VarType=%x (ELEMENT_TYPE_CLASS=%d) Is it a class: %s", i, Class.Fields[i].VarType, ELEMENT_TYPE_CLASS, ELEMENT_TYPE_CLASS==Class.Fields[i].VarType)
          ...
          if Class.Fields[i].VarType==ELEMENT_TYPE_CLASS or 
             Class.Fields[i].VarType==ELEMENT_TYPE_GENERICINST then
    ...
    if dotnetpipe and dotnetpipe.isValid() then      
      ...
      if Address==nil or Address==0 then
        ...
        if Field.Address then
          ...
          return nil,translate('No valid address provided for non-static field')
        end
      else 
        addr=Address+Field.Offset
      end
...
local function getStaticFieldValue(Field)
  if Field.Class.Image.Domain.Control==CONTROL_MONO then
    if Field.Address and (Field.Address~=0) then
     
      if (Field.VarType==ELEMENT_TYPE_STRING) or (Field.VarTypeName == "System.String") then     
        local a=readPointer(Field.Address)
        if a then
    ...
    if Field.Address==nil or Field.Address==0 then
      if dotnetpipe and dotnetpipe.isValid() then
...
local function FieldValueUpdaterTimer(frmDotNetInfo, sender)
  ...
  if Class and frmDotNetInfo.lvStaticFields.TopItem~=nil then
    for i=frmDotNetInfo.lvStaticFields.TopItem.Index, math.min(frmDotNetInfo.lvStaticFields.Items.Count-1, frmDotNetInfo.lvStaticFields.TopItem.Index+frmDotNetInfo.lvStaticFields.VisibleRowCount) do
      ...
      if Class.Fields and (ci>0) and (ci<=#Class.Fields) then
  ...
  if address and Class and address>0x10000 then
    if frmDotNetInfo.lvFields2 then
...
local function lvStaticFieldsDblClick(frmDotNetInfo,sender)
  ...
  if value then
    setFieldValue(Field, Field.Address or 0,value)    
...
local function launchBackgroundParser()
--parse everything
  if (not DataSource.FullyParsed) and (DataSource.BackgroundParser==nil) then
    DataSource.BackgroundParserHeartbeat=0
    DataSource.BackgroundParser=createThread(function(t)
        ...
        for i=1,#DataSource.Domains do
          ...
          for j=1,#DataSource.Domains[i].Images do
            ...
            for k=1,#DataSource.Domains[i].Images[j].Classes do  
              ...
              --todo: queue multiple classes
              
              --get all the fields of each class
              ...
              if k % 10 == 0 then 
                sleep(10) 
                DataSource.BackgroundParserHeartbeat=DataSource.BackgroundParserHeartbeat+1                
              end
            end 
          end
      ...
      if r then
        DataSource.FullyParsed=true
      else
        print('Background parser error:'..err)
      end
      DataSource.BackgroundParser=nil 
      DataSource.BackgroundParserHeartbeat=nil      
    end)
  end
end
...
function miDotNetInfoClick(sender)
  ...
  frmDotNetInfo.Tag=i
  frmDotNetInfo.ProcessID=getOpenedProcessID()
  ...
  frmDotNetInfo.lvClasses.PopupMenu=createPopupMenu(frmDotNetInfo)
  local miExportToStructureDissect=createMenuItem(frmDotNetInfo)
  miExportToStructureDissect.Caption=translate('Export to structure dissect')
  miExportToStructureDissect.OnClick=function(sender)
    local lv=frmDotNetInfo.lvClasses
    if lv.ItemIndex~=-1 then
      local s=createStructureFromName(lv.Items[lv.ItemIndex].Caption)
      if s then        
  ...
  frmDotNetInfo.lvClasses.PopupMenu.Items.add(miExportToStructureDissect)
  ...
  frmDotNetInfo.miEnumAllStatics.OnClick=function()     
    spawnEnumStaticsWindow(DataSource, frmDotNetInfo)     
  end
  ...
  frmDotNetInfo.miChangeColors.Caption=translate('Colors and settings')
  ...
  local miCopyName=createMenuItem(frmDotNetInfo)
  miCopyName.Name='miCopyStaticFieldName'
  miCopyName.Caption=translate('Copy Field Name')
  miCopyName.OnClick=function()
    if frmDotNetInfo.lvStaticFields.Selected then
      local Field=frmDotNetInfo.CurrentlyDisplayedClass.Fields[frmDotNetInfo.lvStaticFields.Selected.Data]
      writeToClipboard(Field.Name)
    end
  end
  frmDotNetInfo.lvStaticFields.PopupMenu.Items.add(miCopyName)
  
  local miCopyTypeName=createMenuItem(frmDotNetInfo)
  miCopyTypeName.Name='miCopyStaticTypeName'  
  miCopyTypeName.Caption=translate('Copy Type Name')
  miCopyTypeName.OnClick=function()
    if frmDotNetInfo.lvStaticFields.Selected then
      local Field=frmDotNetInfo.CurrentlyDisplayedClass.Fields[frmDotNetInfo.lvStaticFields.Selected.Data]
      writeToClipboard(Field.VarTypeName)
    end
  end
  frmDotNetInfo.lvStaticFields.PopupMenu.Items.add(miCopyTypeName)
  
  local miCopyAddress=createMenuItem(frmDotNetInfo)  
  miCopyAddress.Caption=translate('Copy Address')
  miCopyAddress.Name='miCopyStaticAddress'
  miCopyAddress.OnClick=function()
    if frmDotNetInfo.lvStaticFields.Selected then
      local Field=frmDotNetInfo.CurrentlyDisplayedClass.Fields[frmDotNetInfo.lvStaticFields.Selected.Data]
      if Field.Address then
        writeToClipboard(string.format('%x',Field.Address))
      end
    end
  end
  frmDotNetInfo.lvStaticFields.PopupMenu.Items.add(miCopyAddress)  
  
  local miCopyValue=createMenuItem(frmDotNetInfo)  
  miCopyValue.Caption=translate('Copy Value')
  miCopyValue.Name='miCopyStaticValue'
  miCopyValue.OnClick=function()
    if frmDotNetInfo.lvStaticFields.Selected then
      local Field=frmDotNetInfo.CurrentlyDisplayedClass.Fields[frmDotNetInfo.lvStaticFields.Selected.Data]
      writeToClipboard(getStaticFieldValue(Field) or '')
    end
  end
  frmDotNetInfo.lvStaticFields.PopupMenu.Items.add(miCopyValue)    
   ...
   -- frmDotNetInfo.lvFields.Visible=false
    ...
    frmDotNetInfo.lvFields2.OnExpanding=function(sender, node)
     ...
     -- print("Expanding node")  --debug
      ...
      if (not ((Field.VarType==ELEMENT_TYPE_CLASS) or (Field.VarType==ELEMENT_TYPE_GENERICINST))) then
        return nil
      end
      ...
      if frmDotNetInfo.ClassSearcher then
        ...
        if frmDotNetInfo.ClassSearcher.waitfor(2000)==false then
          frmDotNetInfo.ClassSearcher.freeOnTerminate(true) 
        else
      ...
      if Field.VarType==ELEMENT_TYPE_GENERICINST then
        if Field.Class.Image.Domain.Control==CONTROL_MONO then

          if Field.GenericClassName==nil then             
            local fieldclass=mono_field_getClass(Field.Handle)
            if fieldclass then
              --Field.GenericClassName=mono_class_getFullName(fieldclass) 
              Field.GenericClassName=mono_class_getNamespace(fieldclass)..'.'..mono_class_getName(fieldclass)            
            end
          end

          if Field.GenericClassName then
            typename=Field.GenericClassName
          end
        end
      end
      
      
      frmDotNetInfo.ClassSearcher,Class=SearchClassName(typename, 
      function(c) --onFound
  ...
  if #DataSource.Domains==0 then
    ...
    if settings.StartParsingOnFirstShow then
      launchBackgroundParser()
