...
# Fix miniupnpd download failure: update to latest version from openwrt/packages
rm -rf feeds/kenzok8/miniupnpd
git clone --depth 1 --filter=blob:none --sparse https://github.com/openwrt/packages.git temp-openwrt && cd temp-openwrt && git sparse-checkout set net/miniupnpd && cd .. && mv temp-openwrt/net/miniupnpd feeds/kenzok8/miniupnpd && rm -rf temp-openwrt
