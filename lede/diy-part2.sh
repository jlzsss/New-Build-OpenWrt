...
git clone --depth 1 --filter=blob:none --sparse https://github.com/openwrt/packages.git temp-lede && cd temp-lede && git sparse-checkout set utils/dockerd && cd .. && rm -rf feeds/packages/utils/dockerd && mv temp-lede/utils/dockerd feeds/packages/utils && rm -rf temp-lede
