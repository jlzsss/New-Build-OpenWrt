...
git clone --depth 1 --filter=blob:none --sparse https://github.com/openwrt/packages.git temp-lede && cd temp-lede && git sparse-checkout set utils/dockerd utils/containerd && cd .. && rm -rf feeds/packages/utils/dockerd feeds/packages/utils/containerd && mv temp-lede/utils/dockerd temp-lede/utils/containerd feeds/packages/utils/ && rm -rf temp-lede
